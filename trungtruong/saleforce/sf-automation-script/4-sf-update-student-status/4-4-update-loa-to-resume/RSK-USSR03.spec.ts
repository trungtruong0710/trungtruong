...
test.describe('Create resume order fail', () => {
    ...
    test('Resume in the past (resume date < loa date)', async ({ page }) => {
        ...
        const studentUrl = await sFCreateNewStudent.createStudentWithPaymentDetailOnly('RSK-USSR03:1 ', environment.gradeRSK);
    ...
    test('Resume in the future (out of last billing period)', async ({ page }) => {
        ...
        await login.adminSignIn();
        const studentUrl = await sFCreateNewStudent.createStudentWithPaymentDetailOnly('RSK-USSR03:2 ', environment.gradeRSK);
        await sFCommonStep.access_ContactDetail(studentUrl);
        // create enrollment application
        await sFContactDetail.clickBtn_BillingTab();
        await sFContactDetail.clickBtn_ApplicationsTab();
