...
test.describe.skip('Withdrawal student with disable_prorating_flag = true product', () => {
    ...
    test('Cancel product with disable_prorating_flag = true, origin order has billed at order and upcoming billing, eff.date = billing ratio x/y', async ({ page }) => {
        ...
        const studentUrl = await sFCreateNewStudent.createStudentWithPaymentDetailOnly('USSW06:1 ', environment.grade);
        ...
        await sFBilling.assert_Billing_Billing_TotalBillItem(2); // for disable_prorating_flag = true, we dont show the adjustment bill = 0 (logic code)
        await sFBilling.assert_Billing_BillItem(
            [
                ...
                },
                {
                    ...
                    isRatio: true, // by pass
                    billingRatio: billingPeriod.ratio11, // by pass
                    isPackage: true,
                    associateCourses: [
                        {
                            courseName: FP.FP_REQ_1SLOT,
    ...
    test('Cancel product with disable_prorating_flag = true, origin order has upcoming billing only, eff.date = billing ratio 0/y', async ({ page }) => {
        ...
        await login.adminSignIn();
        const studentUrl = await sFCreateNewStudent.createStudentWithPaymentDetailOnly('USSW06:2 ', environment.grade);
        await sFCommonStep.access_ContactDetail(studentUrl);
        // create enrollment application
        await sFContactDetail.clickBtn_BillingTab();
        await sFContactDetail.clickBtn_ApplicationsTab();
