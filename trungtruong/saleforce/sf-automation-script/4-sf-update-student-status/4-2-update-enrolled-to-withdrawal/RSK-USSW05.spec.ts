...
test.describe('Withdrawal student with disable_prorating_flag = false product', () => {
    ...
    test('Cancel product with disable_prorating_flag = false, origin order has billed at order and upcoming billing, eff.date = billing ratio x/y', async ({ page }) => {
        ...
        const studentUrl = await sFCreateNewStudent.createStudentWithPaymentDetailOnly('RSK-USSW05:1 ', environment.gradeRSK);
    ...
    test('Cancel product with disable_prorating_flag = false, origin order has billed at order and upcoming billing, eff.date = billing ratio 0/y', async ({ page }) => {
        ...
        await login.adminSignIn();
        const studentUrl = await sFCreateNewStudent.createStudentWithPaymentDetailOnly('RSK-USSW05:2 ', environment.gradeRSK);
        await sFCommonStep.access_ContactDetail(studentUrl);
        // create enrollment application
        await sFContactDetail.clickBtn_BillingTab();
        await sFContactDetail.clickBtn_ApplicationsTab();
    ...
    test('Cancel product with disable_prorating_flag = false, origin order has upcoming billing only, eff.date = billing ratio 0/y', async ({ page }) => {
        ...
        await login.adminSignIn();
        const studentUrl = await sFCreateNewStudent.createStudentWithPaymentDetailOnly('RSK-USSW05:3 ', environment.gradeRSK);
        await sFCommonStep.access_ContactDetail(studentUrl);
        // create enrollment application
        await sFContactDetail.clickBtn_BillingTab();
        await sFContactDetail.clickBtn_ApplicationsTab();
        ...
        await sFContactDetail.clickBtn_BillingTab();
        await sFContactDetail.clickBtn_ApplicationsTab();
        await sFApplications.createWithdrawalApplication(environment.location, environment.reason, sF_DateTime.startDate_2131Jan01[0]);
        await sFApplications.accessWithdrawalOrder();
        await sFCommonStep.wait(2000);
        await sFCommonStep.click('Submit');
        await sFCommonStep.click('Confirm');
