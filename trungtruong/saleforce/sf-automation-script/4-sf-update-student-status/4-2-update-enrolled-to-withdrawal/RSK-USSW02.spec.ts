...
test.describe('Not able to create withdrawal order with enrollment status is enrolled', () => {

    test('Cannot create withdrawal order with Update schedule tag and last attendance date < eff.date', async ({ page }) => {

        await login.adminSignIn();
        const studentUrl = await sFCreateNewStudent.createStudentWithPaymentDetailOnly('USSW02:6 ', environment.gradeRSK);
    ...
    test('Cannot create withdrawal order with Update schedule tag and last attendance date > eff.date', async ({ page }) => {
        ...
        await login.adminSignIn();
        const studentUrl = await sFCreateNewStudent.createStudentWithPaymentDetailOnly('RSK-USSW02:7 ', environment.gradeRSK);
        await sFCommonStep.access_ContactDetail(studentUrl);
        // create enrollment application
        await sFContactDetail.clickBtn_BillingTab();
        await sFContactDetail.clickBtn_ApplicationsTab();
