...
test.describe.skip('LOA student with disable_prorating_flag = false product', () => {
    ...
    test('LOA product with disable_prorating_flag = false, origin order has billed at order and upcoming billing, eff.date = billing ratio x/y', async ({ page }) => {
        ...
        const studentUrl = await sFCreateNewStudent.createStudentWithPaymentDetailOnly('USSL04:1 ', environment.grade);
    ...
    test('LOA product with disable_prorating_flag = false, origin order has billed at order and upcoming billing, eff.date = billing ratio 0/y', async ({ page }) => {
        ...
        await login.adminSignIn();
        const studentUrl = await sFCreateNewStudent.createStudentWithPaymentDetailOnly('USSL04:2 ', environment.grade);
        await sFCommonStep.access_ContactDetail(studentUrl);
        // create enrollment application
        await sFContactDetail.clickBtn_BillingTab();
        await sFContactDetail.clickBtn_ApplicationsTab();
        ...
        await sFContactDetail.clickBtn_BillingTab();
        await sFContactDetail.clickBtn_ApplicationsTab();
        await sFApplications.createLOAApplication(environment.location, environment.reason, sF_DateTime.startDate_2041Jan01[0], sF_DateTime.startDate_2051Jan01[0]);
        await sFApplications.accessLOAOrder();
        await sFCommonStep.wait(2000);
        await sFCommonStep.click('Submit');
        await sFCommonStep.click('Confirm');
            ...
            [
                {
                    ...
                    isRecurringProduct: true,
                    billingPeriod: billingPeriod.period_2021_2050,
                    isRatio: false, // by pass
                    billingRatio: null, // by pass
                    isPackage: true,
                    associateCourses: [
                        {
                            courseName: FP.FP_REQ_1SLOT,
    ...
    test.skip('LOA product with disable_prorating_flag = false, origin order has upcoming billing only, eff.date = billing ratio 0/y', async ({ page }) => {

        await login.adminSignIn();
        const studentUrl = await sFCreateNewStudent.createStudentWithPaymentDetailOnly('USSL04:3 ', environment.grade);
        await sFCommonStep.access_ContactDetail(studentUrl);
        // create enrollment application
        await sFContactDetail.clickBtn_BillingTab();
        await sFContactDetail.clickBtn_ApplicationsTab();
