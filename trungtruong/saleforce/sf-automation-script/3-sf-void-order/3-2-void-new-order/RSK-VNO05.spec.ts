...
test.describe('Void recurring product order incase invoice order successfully and payment status is sucessful (disable_prorating_flag = true)', () => {

    test('Void order with payment status = Successful (Cash)', async ({ page }) => {

        await login.adminSignIn();
        const studentUrl = await sFCreateNewStudent.createStudent('RSK-VNO05:1 ', environment.gradeRSK);
    ...
    test('Void order with payment status = Successful (Bank Transfer)', async ({ page }) => {
        ...
        await login.adminSignIn();
        const studentUrl = await sFCreateNewStudent.createStudent('RSK-VNO05:2 ', environment.gradeRSK);
        await sFCommonStep.access_ContactDetail(studentUrl);
        await sFCreateOrder.createOrder(
            {
                location: environment.location,
