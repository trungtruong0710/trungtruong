...
test.describe('Void enrollment order successfully after bill item is invoice and payment status is sucessful (disable_prorating_flag = true)', () => {

    test('Void enrollment order with payment status = Successful (Cash)', async ({ page }) => {

        await login.adminSignIn();
        const studentUrl = await sFCreateNewStudent.createStudentWithPaymentDetailOnly('RSK-VEO07:1 ', environment.gradeRSK);
    ...
    test('Void enrollment order with payment status = Successful (Bank Transfer)', async ({ page }) => {
        ...
        await login.adminSignIn();
        const studentUrl = await sFCreateNewStudent.createStudentWithPaymentDetailOnly('RSK-VEO07:2 ', environment.gradeRSK);
        await sFCommonStep.access_ContactDetail(studentUrl);
        // create enrollment application
        await sFContactDetail.clickBtn_BillingTab();
        await sFContactDetail.clickBtn_ApplicationsTab();
