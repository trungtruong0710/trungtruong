...
test.describe('Void enrollment order with start date = current date (student enrollment status = enrolled)', () => {
    ...
    test('Void with eff.date = current date', async ({ page }) => {
        ...
        const studentUrl = await sFCreateNewStudent.createStudentWithPaymentDetailOnly('VEO02:1 ', environment.grade);
    ...
    test('Void with eff.date = future date', async ({ page }) => {

        await login.adminSignIn();
        const studentUrl = await sFCreateNewStudent.createStudentWithPaymentDetailOnly('VEO02:2 ', environment.grade);
        await sFCommonStep.access_ContactDetail(studentUrl);
        // create enrollment application
        await sFContactDetail.clickBtn_BillingTab();
        await sFContactDetail.clickBtn_ApplicationsTab();
